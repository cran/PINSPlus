...
author: |
  | Bang Tran, Duc Tran, Hung Nguyen and Tin Nguyen$^*$
...
abstract: PINS+ provides a robust approach for data integration and disease subtyping.
  ...
  such that the results are robust to noise and data perturbation. PINS+ has been
  validated on thousands of cancer samples obtained from the Gene Expression Omnibus,
  the Broad Institute, The Cancer Genome Atlas (TCGA), the European Genome-Phenome
  Archive and simulation data. The approach can accurately identify known subtypes
  and discover novel groups of patients with significantly different survival profiles.
  The software is extremely fast and able to cluster hundreds of matched samples in
  minutes.
vignette: |
  %\VignetteEngine{knitr::knitr} %\VignetteIndexEntry{PINSPlus} %\usepackage[UTF-8]{inputenc}
---
